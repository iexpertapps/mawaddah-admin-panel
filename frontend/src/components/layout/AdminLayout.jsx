    ...
    <aside className="hidden lg:flex flex-col w-64 h-full bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 shadow-lg">
      <div className="flex items-center h-20 px-6 border-b border-gray-200 dark:border-gray-700">
        <img
          ...
          draggable="false"
        />
        <span className="ml-3 text-2xl font-bold bg-gradient-to-r from-primary to-accent bg-clip-text text-transparent tracking-tight">
          Mawaddah
        </span>
      </div>
        ...
        {navigation.map((item) => {
          ...
          return (
            <NavLink
              key={item.name}
              to={item.href}
              className={`group flex items-center px-4 py-2 text-base font-medium rounded-lg transition-all duration-200 gap-3 ${
                isActive
                  ? 'bg-primary/10 text-primary border-l-4 border-primary shadow-md dark:bg-primary/20 dark:text-primary'
                  : 'text-gray-700 dark:text-gray-300 hover:bg-primary/5 hover:text-primary dark:hover:bg-primary/10 dark:hover:text-primary'
              }`}
            >
              <item.icon
                className={`w-6 h-6 ${
                  isActive
                    ? 'text-primary drop-shadow-[0_0_6px_rgba(26,127,85,0.4)]'
                    : 'text-gray-400 group-hover:text-primary dark:text-gray-500 dark:group-hover:text-primary'
                }`}
              />
              {item.name}
            </NavLink>
